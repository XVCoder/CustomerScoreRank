...
namespace CustomerScoreRank.Lib.DataStructures
{
    public class SkipList<T> : ICollection<T>
    {
        ...
        public SkipList(IComparer<T> comparer)
        {
            ...
            _head = new Node(default, HEIGHT_STEP);
            _tail = new Node(default, HEIGHT_STEP);
            Reset();
        }
        ...
        public virtual bool Exist(T item) => FindNode(item) != null;
        ...
        /// <summary>
        /// 获取当前项的前previousItemCount个，后nextItemCount个项
        /// </summary>
        /// <param name="currentItem">当前项</param>
        /// <param name="previousItemCount">前驱项数</param>
        /// <param name="nextItemCount">后继项数</param>
        /// <param name="includeCurrentItem">是否包含当前项</param>
        /// <returns></returns>
        public virtual IEnumerable<T> GetRangeAroundItem(T currentItem, int previousItemCount, int nextItemCount, bool includeCurrentItem = true)
        {
            if (currentItem == null)
                return new LinkedList<T>();

            if (previousItemCount < 0 || currentItem.Equals(_head.Item))
                previousItemCount = 0;

            if (nextItemCount < 0 || currentItem.Equals(_tail.Item))
                nextItemCount = 0;

            Node currentNode = FindNode(currentItem);
            ...
            if (includeCurrentItem)
                result.AddFirst(currentItem);

            Node tempNode = currentNode;
            while (previousItemCount-- > 0)
            {
                tempNode = tempNode.GetPrev(0);
                if (tempNode.Item == null) break;
                result.AddFirst(tempNode.Item);
            }
            tempNode = currentNode;
            while (nextItemCount-- > 0)
            {
                tempNode = tempNode.GetNext(0);
                if (tempNode.Item == null) break;
                result.AddLast(tempNode.Item);
            }

            return result;
        }

        public virtual IEnumerable<T> GetRangeByIndex(int from, int to)
        {
            if (from < 0) from = 0;
            ...
            if (from > to)
                throw new Exception("[from] can not be bigger than [to]");

            var fromNode = FindNodeByIndex(from);

            return GetRangeAroundItem(fromNode.Item, 0, to - from);
        }
        ...
        protected Node FindNodeByIndex(int index)
        {
            if (index < 0) return null;

            Node target = FindNode(GetFirst());

            while (index-- > 0)
            {
                target = target.GetNext(0);
            }
            return target;
        }
